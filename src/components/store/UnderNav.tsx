...
const UnderNav = ({ sortBy, onSortChange, resultCount }: UnderNavProps) => {
  ...
  const currentLabel = sortOptions.find((o) => o.value === sortBy)?.label ?? "Recommended";

  return (
    <section className="shrink-0 border-b border-slate-200 px-6 py-5">
      <nav aria-label="Breadcrumb" className="mb-2 flex items-center gap-1.5 text-sm text-slate-500">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <div className="flex items-baseline gap-2.5">
            <h2 className="text-3xl font-semibold tracking-tight text-slate-900">Computing gear</h2>
            <span className="text-sm text-slate-400">{resultCount.toLocaleString()} results</span>
          </div>
          <p className="mt-1 text-sm text-slate-500">Browse computing products</p>
        </div>
              ...
              >
                {sortOptions.map((option) => (
                  <li key={option.value}>
                    ...
                    >
                      {option.label}
                      {sortBy === option.value && <Check className="size-4 text-indigo-600" />}
                    </button>
                  </li>
                ))}
              </ul>
